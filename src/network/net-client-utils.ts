...
export function printAxiosResponseLog(requestConfig: INetRequestConfig, iNetResponse: INetResponse) {
  console.log('NetClient RESPONSE', {
    method: requestConfig.method,
    url: requestConfig.url,
    request: {
      headers: requestConfig.headers,
...
export function printAxiosErrorLog(requestConfig: INetRequestConfig, error: any) {
  ...
  if (error.isAxiosError) {
    ...
    if (error.response) {
      ...
      if (error.code) {
        console.log('NetClient ERROR (AXIOS, NO_RESPONSE, CODE)', {
          status: 500,
        ...
        console.log('NetClient ERROR (AXIOS, NO_RESPONSE, NO_CODE)', error, {
          req: logData,
        });
      }
    }
